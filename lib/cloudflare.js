...
export const getLists = async () => {
  const res = await cfFetch("/lists");
  console.log("getLists raw response:", JSON.stringify(res));
  return res;
};
...
export async function syncLists(domains) {
  ...
  const { result: allLists, errors } = await getLists();
  if (!allLists) {
    throw new Error("Failed to fetch lists. Check CLOUDFLARE_API_TOKEN and CLOUDFLARE_ACCOUNT_ID. Errors: " + JSON.stringify(errors));
  }
